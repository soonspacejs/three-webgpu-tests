...
import { fromBufferGeometryLike, toBufferGeometryLike } from '@takram/three-geospatial';

import { queueTask } from '../worker/pool';

async function toCreasedNormalsAsync(geometry: BufferGeometry, creaseAngle?: number): Promise<BufferGeometry> {
  const [geometryLike, transfer] = toBufferGeometryLike(geometry);
  const result = await queueTask('toCreasedNormals', [geometryLike, creaseAngle], { transfer });
  ...
  async processTileModel(scene: Object3D): Promise<void> {
    ...
    await Promise.all(
      meshes.map(async (mesh) => {
        mesh.geometry = await toCreasedNormalsAsync(mesh.geometry, this.options.creaseAngle);
      })
    );
  }
}
